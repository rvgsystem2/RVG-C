        ...
        <div class="row mb-4 wow fadeInUp" data-wow-delay="0.2s">
            <div class="col-12 d-flex justify-content-center flex-wrap gap-2" id="portfolio-flters">
                ...
                @foreach ($categories as $category)
                    <button class="btn btn-outline-dark" data-filter=".{{ Str::slug($category) }}">{{ $category }}</button>
                @endforeach
            </div>
        </div>
                ...
                <div class="col-lg-4 col-md-6 portfolio-item {{ Str::slug($project->project_category_name) }} wow fadeInUp" data-wow-delay="0.1s">
                    <div class="card shadow-sm border-0 h-100 project-card overflow-hidden rounded-4">
                        <div class="position-relative">
                            <img src="{{ asset('storage/' . $firstImage) }}" class="card-img-top" alt="{{ $project->title }}" style="height: 240px; object-fit: cover;">
                            <div class="portfolio-overlay d-flex align-items-center justify-content-center gap-2">
                                ...
                                @if ($project->project_url)
                                    <a href="{{ $project->project_url }}" target="_blank" class="btn btn-light btn-sm rounded-circle">
