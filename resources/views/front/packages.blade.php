            ...
            @forelse($packages as $pkg)
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="card border-0 shadow-sm h-100 pricing-card position-relative">
                        ...
                        <div class="bg-white rounded border d-flex align-items-center justify-content-center"
                            style="min-height: 180px;">
                            @if ($thumb)
                                <img src="{{ asset('storage/' . $thumb) }}" alt="{{ $pkg->image_alt ?? $pkg->name }}"
                                    class="img-fluid d-block" style="max-height:220px; object-fit:contain;" loading="lazy">
                            @else
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mb-2">
                                ...
                                @if ($hasSale)
                                    <span
                                        class="text-muted text-decoration-line-through ms-2">₹{{ number_format((float) $pkg->price, 2) }}</span>
                                    <span class="badge text-bg-success ms-2">Save
                                        ₹{{ number_format((float) ($pkg->price - $pkg->sale_price), 2) }}</span>
                                @endif
                            </div>
                            ...
                            <div class="mt-auto d-grid gap-2">
                                ...
                                <a href="{{ route('packages.details', Str::slug($pkg->name)) }}"
                                    class="btn btn-primary rounded-pill py-2">
                                    View Details
                                </a>
